...
def calculate_recovery_time_ms(gateway: ReliabilityGateway) -> float | None:
    ...
    Walks each breaker's transition_log entries, tracks open→close cycles,
    and returns the average recovery time in milliseconds, or None if no recovery occurred.
    """
    ...
    for breaker in gateway.breakers.values():
        ...
        for entry in breaker.transition_log:
            if entry["to"] == "open" and open_ts is None:
                ...
                recovery_ms = (close_ts - open_ts) * 1000.0
                recovery_times.append(recovery_ms)
                open_ts = None  # reset for next open→close cycle
...
def run_scenario(config: LabConfig, queries: list[str], scenario: ScenarioConfig) -> RunMetrics:
    ...
    for _ in range(config.load_test.requests):
        ...
        else:  # primary or other success
            metrics.successful_requests += 1

        if result.latency_ms > 0:
            metrics.latencies_ms.append(result.latency_ms)

    # Count circuit opens from transition logs
    for breaker in gateway.breakers.values():
        metrics.circuit_open_count += sum(1 for t in breaker.transition_log if t["to"] == "open")
...
def run_simulation(config: LabConfig, queries: list[str]) -> RunMetrics:
    ...
    for scenario in config.scenarios:
        ...
        if scenario.name == "primary_timeout_100":
            ...
            passed = result.circuit_open_count >= 1
        elif scenario.name == "all_healthy":
            passed = result.error_rate < 0.1
        elif scenario.name == "primary_slow_80":
            passed = result.recovery_time_ms is not None
